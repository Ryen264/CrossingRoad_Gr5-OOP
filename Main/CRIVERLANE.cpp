...
void CRIVERLANE::Move() {
	...
	if (this->isMoveRight) {
		...
		if (rand() % 10 == 0) {
			if (rand() % 2 == 0)
				this->lane.push_front(new CPERRY(0, this->y, true));
			else
				this->lane.push_front(new CCAPYBARA(0, this->y, true));
		}
		else
			this->lane.push_front(NULL);
	}
		...
		if (rand() % 10 == 1) {
			if (rand() % 2 == 0)
				this->lane.push_back(new CPERRY(0, this->y, false));
			else
				this->lane.push_back(new CCAPYBARA(0, this->y, false));
		}
		else
...
void CRIVERLANE::setStop(bool isStop)
{
	this->isStop = isStop;
}
